...
assert.match(uiSource, /function compareClassRosterItems[\s\S]*\['北區', '東橋'\][\s\S]*一二三四五六日[\s\S]*a\.start/, '班級需先按分校，再按星期與上課時間排序');
assert.match(uiSource, /filter[\s\S]*\.sort\(compareClassRosterItems\)/, '分校與搜尋篩選後仍需維持固定排序');
...
assert.match(cssSource, /\.roster-reminder-summary\.is-overdue/, '逾期提醒需有整列強警示');
assert.match(cssSource, /@media \(max-width: 360px\)[\s\S]*\.roster-campus-overview \{ grid-template-columns: 1fr;/, '最窄手機需改為單欄分校摘要');
assert.match(uiSource, /if \(items\.length <= 1\) return '';/, '單一頁面工作區不應顯示佔滿底部的無效導覽列');
assert.match(uiSource, /isRosterOnly \? '班級人數管理'/, '獨立班級頁標題不得誤寫成行政美宣工作台');
assert.match(uiSource, /action === 'open-class-reminders'[\s\S]*classView = 'reminders'[\s\S]*classQuery = ''/, '總提醒入口需清除搜尋並顯示完整待辦');
