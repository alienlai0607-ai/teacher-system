# KPI 系統部署與更新

本文件適用於正式站 `https://teacher.blockplanetcamp.com/`。既有資料庫更新時不要刪除工作表，也不要重新建立空白試算表。

## 1. 更新 Apps Script

1. 開啟 KPI 系統的 Apps Script 專案。
2. 將 `apps-script/_all_in_one.gs` 的完整內容更新到專案程式碼。
3. 儲存後手動執行 `setupSheets()`。
4. 執行 `prepareAdminMarketingLaunch()`，保留既有資料並加入行政美宣工作表與角色設定。
5. 確認共 17 個資料表，並確認 `AdminMarketingRecords` 已建立、舊「永康教室」已轉為「東橋教室」。
6. 執行 `setupSystemAutomation()`，確認排程與提醒均建立成功。
7. 「部署 → 管理部署作業 → 編輯 → 新版本 → 部署」。
8. 保持執行身分為擁有者、存取權限為任何人；資料 API 仍由系統簽章工作階段保護。

首次建立時可在 Google Sheet 的「擴充功能 → Apps Script」貼上同一份合併檔，再依上述步驟執行。初始化只預建人員暱稱，不會替使用者填入 Email。

## 2. 首位管理員與新人

系統不允許使用者自行認領暱稱。

1. 首次安裝後，在 `Users` 表手動填入柏翰的 Google Email。
2. 柏翰登入後，到人員管理替每位新人建立或更新：暱稱、Email、角色、教室、狀態。
3. 新人使用完全相同的 Google Email 登入。
4. 短期暫停使用時選「停用」；確定離職時只由柏翰在「人員管理」按「刪除員工」，並再次輸入完整暱稱確認。
5. 「刪除員工」會立即移除登入、Google、LINE、APP 與電話綁定；歷史日報、薪資、評分及雲端 PDF 仍保留供主管查核。不要直接刪除 `Users` 工作表列。

## 3. 必要設定

`shared/config.js`：

```js
API_URL: 'Apps Script Web App /exec 網址',
GOOGLE_CLIENT_ID: 'Google OAuth 網頁用戶端 ID',
```

Google OAuth 的 JavaScript 來源至少包含：

```text
https://teacher.blockplanetcamp.com
http://127.0.0.1:8788
```

Apps Script「指令碼屬性」需視功能設定：

| 屬性 | 用途 |
|---|---|
| `GOOGLE_CLIENT_ID` | 後端核對 Google ID token 的 audience |
| `LINE_TOKEN` | LINE Messaging API push token |
| `ONESIGNAL_APP_ID` | OneSignal Web Push App ID |
| `ONESIGNAL_REST_KEY` | OneSignal REST API key |
| `LINE_CHANNEL_SECRET` | LINE webhook 維運識別（若部署環境可取得簽章標頭） |

`API_SESSION_SECRET`、PDF 資料夾 ID 等值由後端在需要時建立，不要公開到前端。

## 4. 發布前端

1. 更新所有修改過的 JS/CSS 查詢版本號，避免 iPhone 或 LINE 內建瀏覽器沿用舊檔。
2. 推送 `main` 到正式 GitHub Pages 儲存庫。
3. 等待 Pages 發布完成。
4. 用無痕視窗與手機重新開啟正式入口，不要只測已登入分頁。

## 5. 上線檢查

每次發布前先執行 `sh tests/release-gate.sh`。部署後再由柏翰於管理員系統健康檢查執行「正式雲端交付驗收」，同一次結果必須為 `5/5`；此驗收會真的新增、修改、讀回並清除安親紀錄，也會實際建立、讀取及清除照片與教材。完整標準見 `docs/RELEASE_ACCEPTANCE.md`。任一項失敗時不得回報完成。

- `ping` 回傳 `ok: true`。
- 柏翰登入顯示管理員本人，不沿用酸酸或其他代看身分。
- 酸酸只能看東橋；小魚可看東橋與北區；老師只能看與改自己的資料。
- 皮皮登入可在「才藝 PT」與「行政美宣」間切換，不需要重新登入；兩個工作區的紀錄與計算互不混用。
- 小魚可進入「行政美宣主管」，柏翰可使用相同主管視角；其他老師看不到行政美宣主管資料。
- 行政美宣每日訊息確認、未完成期限、完成日期與美宣完成證據均有後端必填驗證。
- 行政美宣每週影片 2 支、照片宣傳 3 則由每日完成紀錄自動彙整，不需月底重填。
- 行政美宣證據可一次選多檔；主管可從「雲端資料」開啟皮皮資料夾與近期附件。
- 行政美宣月度評核為 100 分制，主管建議公布後皮皮可查看並在同一對話串回覆。
- 老師能建立草稿、關閉後重開、正式送出、重開已送出紀錄。
- 安親紀錄送出後如有任何修改，狀態立即改為「待重新送出」；重新確認送出後主管才會收到新版本。
- 才藝老師可從「我的紀錄」編輯當日紀錄；PT 一般課堂跨日後不可修改，停課仍可補登。
- 學科內與學科外選項不互相混入，必填欄位都有紅色星號。
- 多張照片可一次選取並逐張移除，照片重點標記可個別刪除；非圖片檔案可上傳，失敗時本機草稿仍保留。
- 才藝備課只需課程類型、課程名稱及至少一份教案或教材附件；附件可一次選擇文件、照片與 MP4／MOV 影片，超過 25 MB 時在讀取前明確阻擋。
- 才藝備課儲存後立即可於本堂紀錄選用，主管端只供依老師查閱，不得再出現送審、核准、退回或完成度門檻。
- 班務四項各有照片，控制保持精簡。
- 課程可選備課教案檔案；班級經營不要求備課檔案。
- 正式送出後 PDF 建立，主管 APP／LINE 回應顯示真實送達狀態。
- 老師能查看主管評分、主管建議並在同一對話串回覆。
- 月歸檔路徑為 `KPI月歸檔 / 教室 / 老師 / YYYY-MM`。
- 才藝月結可依月份保留已離職人員的既有堂次與金額，現職人員及排班頁不再顯示離職者。
- 柏翰能看到「刪除員工」；其他帳號不能刪除員工、目前管理員不能刪除自己或其他管理員。
- 刪除後舊工作階段立即失效，已刪除人員不能新增、補寫或修改任何紀錄。
- 雲端日報、教材、證據與月歸檔採私人權限；刪除員工時收回其 Google Drive 權限。
- 舊 PDF 可查閱，但不轉成新版日誌。
- 桌面與 390px 手機寬度無橫向溢位、按鈕可點、文字不重疊。

## 6. 故障判斷

### 登入後仍是舊介面

先確認 HTML 引用的查詢版本號已更新，再開無痕視窗測試。登入入口會清除代看狀態並向後端重新核對真實帳號；若仍錯誤，檢查 `Users.email` 是否綁錯人。

### 白畫面或資料載入失敗

查看瀏覽器 Console 與 Network，確認 JS/CSS 沒有 404、Apps Script 部署仍有效、`API_URL` 為最新 `/exec`。本機草稿不應因 API 失敗被清除。

### APP／LINE 測試失敗

到「帳號與通知」分別檢查 APP 訂閱與 LINE 綁定。瀏覽器封鎖通知時需從網站設定改為允許；LINE 指令必須使用系統產生、10 分鐘有效的專屬綁定碼。

### 新人無法登入

在管理介面確認 Email 完全一致、角色與教室正確、狀態為 `active`。系統不提供自助認領。

## 7. 禁止的維運操作

- 不要為了重跑初始化而刪除所有工作表。
- 不要直接刪除 `Users` 列、離職者舊日誌、薪資、評分或 PDF；使用柏翰管理頁的「刪除員工」。
- 不要把測試帳號綁到正式老師 Email。
- 不要只改前端角色判斷而省略後端授權。
- 不要宣稱通知成功，除非 API 回傳該收件人至少一個管道送達。
